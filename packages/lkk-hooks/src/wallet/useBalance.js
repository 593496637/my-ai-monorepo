...
const useBalance = (account, chainId, tokenAddress = null) => {
  const [balance, setBalance] = useState('0');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  // 获取ETH余额
  const getETHBalance = useCallback(async (address) => {
  ...
  const getTokenBalance = useCallback(async (address, tokenAddress) => {
